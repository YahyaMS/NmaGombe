...
export function SignupForm() {
  ...
  async function completeSignIn(email: string) {
    try {
      ...
      // the existing account by email rather than creating a duplicate). Route
      // by role rather than dumping everyone on /pending regardless of status.
      const token = await user.getIdTokenResult(true)
      router.replace(token.claims.role === 'admin' ? '/admin/verification' : '/')
    } catch (err) {
