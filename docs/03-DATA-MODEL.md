...
| No bulk-read endpoint returning every member's data in one call | Implemented | `tests/rules/firestore.test.ts` — the "list/query semantics" section (ADR-033) proves this at the `list` level specifically, not just via `get`: an unconstrained query against `members` fails for anyone but admin, and `directoryEntries`'/`jobs`'/`documents`' doc-independent list grants are exactly the app's own intended usage, not an accidental opening. |
